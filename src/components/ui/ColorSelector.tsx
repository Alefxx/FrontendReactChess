      ...
      {options.map(({ id, label, description }) => {
        ...
        return (
          <button
            ...
            className={`surface-interactive relative flex min-h-20 items-center gap-3 rounded-2xl p-3 text-left disabled:cursor-not-allowed disabled:opacity-35 ${
              isSelected ? 'border-chess-green bg-lime-400/12 ring-2 ring-chess-green/45 shadow-[0_8px_24px_rgba(163,230,53,0.13)]' : ''
            }`}
          >
            {isSelected && <CheckCircle2 aria-hidden="true" className="absolute right-2.5 top-2.5 text-chess-green" size={18} />}
